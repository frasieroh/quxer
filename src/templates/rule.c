...
/*
1   arena_idx_t eval_<name>(memo_state_t* state, char* text,
                       uint32_t text_length, uint32_t pos);
*/
{
    char* name = rule->name;
    fprintf(file,
/*1*/   "arena_idx_t eval_%s(memo_state_t* state, uint8_t* text,\n"
        "                 uint32_t text_length, uint32_t pos);\n",
/*1*/   name);
    return;
}
...
3       rnode_t* result_<root_id> = NULL;
        // child parser
4       return prealloc_idx_<root_id>;
    }
*/
{
